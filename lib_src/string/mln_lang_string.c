...
static int mln_lang_string_capitalize_handler(mln_lang_ctx_t *ctx, mln_lang_object_t *obj);
...
static mln_lang_var_t *mln_capitalize_process(mln_lang_ctx_t *ctx);
...
static int mln_lang_string_join_process_iterate_handler(mln_rbtree_node_t *node, struct mln_lang_string_join_s *lsj);

static int mln_lang_string_global_init(void)
{
    mln_conf_t *cf;
...
mln_lang_var_t *init(mln_lang_ctx_t *ctx)
{
    if (!melon_init_flag) {
        ...
        cattr.argc = 0;
        cattr.argv = NULL;
        cattr.global_init = mln_lang_string_global_init;
#if !defined(WINNT)
...
static int mln_lang_string(mln_lang_ctx_t *ctx, mln_lang_object_t *obj)
{
    ...
    if (mln_lang_string_capitalize_handler(ctx, obj) < 0) {
        return -1;
    }
    return 0;
}
...
static int mln_lang_string_capitalize_handler(mln_lang_ctx_t *ctx, mln_lang_object_t *obj)
{
    ...
    mln_lang_func_detail_t *func;
    mln_string_t v1 = mln_string("s");
    mln_string_t funcname = mln_string("capitalize");

    if ((func = mln_lang_func_detail_new(ctx, M_FUNC_INTERNAL, mln_capitalize_process, NULL, NULL)) == NULL) {
        mln_lang_errmsg(ctx, "No memory.");
        return -1;
    }
    if ((val = mln_lang_val_new(ctx, M_LANG_VAL_TYPE_NIL, NULL)) == NULL) {
...
static mln_lang_var_t *mln_capitalize_process(mln_lang_ctx_t *ctx)
{
    ...
    if (tmp > 96 && tmp < 123)
        s->data[0] -= 32;
    ret_var = mln_lang_var_create_string(ctx, mln_lang_var_val_get(sym->data.var)->data.s, NULL);
    if (tmp != 255)
        s->data[0] = tmp;
    if (ret_var == NULL) {
        mln_lang_errmsg(ctx, "No memory.");
