...
/*components*/
MLN_CHAIN_FUNC_DECLARE(static inline, mln_lang_mq_msg, mln_lang_mq_msg_t,);
MLN_CHAIN_FUNC_DECLARE(static inline, mln_lang_mq_wait, mln_lang_mq_wait_t,);
...
int mln_lang_msgqueue_impl(mln_lang_ctx_t *ctx, mln_lang_object_t *obj)
{
...
static int mln_lang_msgqueue_resource_register(mln_lang_ctx_t *ctx)
{
    ...
    if ((mq_timeout_set = mln_lang_resource_fetch(ctx->lang, "mq_timeout")) == NULL) {
        if ((mq_timeout_set = mln_fheap_new_fast(&mq_wait_min, \
                                                 (fheap_cmp)mln_lang_mq_wait_cmp, \
                                                 (fheap_copy)mln_lang_mq_wait_copy, \
                                                 NULL)) == NULL)
        {
            mln_lang_errmsg(ctx, "No memory.");
            return -1;
    ...
    switch (msg->type) {
        ...
        case M_LANG_VAL_TYPE_REAL:
            *ret_var = mln_lang_var_create_real(ctx, msg->data.f, NULL);
            break;
        case M_LANG_VAL_TYPE_STRING:
            *ret_var = mln_lang_var_create_string(ctx, msg->data.s, NULL);
            break;
        default:
...
static mln_lang_var_t *mln_lang_mq_msg_get(mln_lang_ctx_t *ctx, mln_string_t *qname, mln_s64_t timeout)
{
    ...
    if (mq->wait_head == wait && (msg = mq->msg_head) != NULL) {
        ...
        if (mq->msg_head == NULL && mq->wait_head == NULL) {
            if (rn == NULL) {
                rn = mln_rbtree_search(mq_set, mq);
...
static void mln_lang_msgqueue_timeout_handler(mln_event_t *ev, void *data)
{
    ...
    while (1) {
        ...
        if (mq->msg_head == NULL && mq->wait_head == NULL) {
            mln_rbtree_node_t *rn = mln_rbtree_search(mq_set, mq);
            if (!mln_rbtree_null(rn, mq_set)) {
                mln_rbtree_delete(mq_set, rn);
                mln_rbtree_node_free(mq_set, rn);
            }
...
mln_lang_var_t *mln_lang_mq_msg_set(mln_lang_ctx_t *ctx, mln_string_t *qname, int type, void *data)
{
    mln_lang_t *lang = ctx->lang;
    mln_rbtree_node_t *rn;
    mln_rbtree_t *mq_set = mln_lang_resource_fetch(lang, "mq");
    ...
    if ((wait = mq->wait_head) != NULL) {
        ...
        switch (msg->type) {
            ...
            case M_LANG_VAL_TYPE_REAL:
                ...
                break;
            case M_LANG_VAL_TYPE_STRING:
                ret_var2 = mln_lang_var_create_string(wait->ctx, msg->data.s, NULL);
                break;
            default:
                mln_lang_errmsg(ctx, "Invalid type.");
                ASSERT(0);
...
MLN_CHAIN_FUNC_DEFINE(static inline, mln_lang_mq_msg, mln_lang_mq_msg_t, prev, next);
MLN_CHAIN_FUNC_DEFINE(static inline, mln_lang_mq_wait, mln_lang_mq_wait_t, prev, next);
