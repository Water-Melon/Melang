...
static int mln_lang_sys_utctime_handler(mln_lang_ctx_t *ctx, mln_lang_object_t *obj);
static mln_lang_var_t *mln_lang_sys_utctime_process(mln_lang_ctx_t *ctx);
...
static inline mln_lang_sys_exec_t *
mln_lang_sys_exec_new(mln_lang_ctx_t *ctx, \
                      mln_rbtree_t *tree, \
                      int fd, \
...
static inline mln_lang_ctx_sys_exec_t *mln_lang_ctx_sys_exec_new(mln_lang_ctx_t *ctx, int fd);
static void mln_lang_ctx_resource_sys_exec_free(mln_lang_ctx_sys_exec_t *cse);
...
static inline mln_lang_var_t *mln_lang_sys_exec_process_info(mln_lang_ctx_t *ctx);
static int mln_lang_sys_exec_process_info_iterate(mln_rbtree_node_t *node, void *udata);
...
static int mln_lang_sys(mln_lang_ctx_t *ctx, mln_lang_object_t *obj)
{
    ...
    if (mln_lang_sys_utctime_handler(ctx, obj) < 0) goto err;
...
static int mln_lang_sys_resource_register(mln_lang_ctx_t *ctx)
{
    ...
    mln_lang_ctx_sys_exec_t *se;

    if ((tree = mln_lang_resource_fetch(ctx->lang, "sys_exec")) == NULL) {
        ...
        if (mln_lang_resource_register(ctx->lang, "sys_exec", tree, (mln_lang_resource_free)mln_rbtree_free) < 0) {
            mln_lang_errmsg(ctx, "No memory.");
            mln_rbtree_free(tree);
            return -1;
        }
    }
    if (mln_lang_ctx_resource_fetch(ctx, "sys_exec") == NULL) {
        if ((se = mln_lang_ctx_sys_exec_new(ctx, -1)) == NULL) {
            mln_lang_errmsg(ctx, "No memory.");
            return -1;
        }
        if (mln_lang_ctx_resource_register(ctx, "sys_exec", se, (mln_lang_resource_free)mln_lang_ctx_resource_sys_exec_free) < 0) {
            mln_lang_errmsg(ctx, "No memory.");
            mln_lang_ctx_resource_sys_exec_free(se);
            return -1;
        }
    }
...
static int mln_lang_sys_array_add_elem(mln_rbtree_node_t *node, void *udata)
{
    ...
    mln_lang_var_t *prop = (mln_lang_var_t *)mln_rbtree_node_data_get(node);
...
static int mln_lang_sys_obj_add_key(mln_rbtree_node_t *node, void *udata)
{
    ...
    mln_lang_array_elem_t *ae = (mln_lang_array_elem_t *)mln_rbtree_node_data_get(node);
...
static int mln_lang_sys_utctime_handler(mln_lang_ctx_t *ctx, mln_lang_object_t *obj)
{
    ...
    mln_lang_var_t *var;
    mln_lang_func_detail_t *func;
    mln_string_t funcname = mln_string("utctime");
    mln_string_t v1 = mln_string("tm");
    if ((func = mln_lang_func_detail_new(ctx, M_FUNC_INTERNAL, mln_lang_sys_utctime_process, NULL, NULL)) == NULL) {
        mln_lang_errmsg(ctx, "No memory.");
        return -1;
    }
    if ((val = mln_lang_val_new(ctx, M_LANG_VAL_TYPE_NIL, NULL)) == NULL) {
...
static mln_lang_var_t *mln_lang_sys_utctime_process(mln_lang_ctx_t *ctx)
{
    ...
    mln_time2utc((time_t)(mln_lang_var_val_get(sym->data.var)->data.i), &utc);
    n = snprintf((char *)buf, sizeof(buf) - 1, "%02ld/%02ld/%ld %02ld:%02ld:%02ld UTC", \
                 utc.month, utc.day, utc.year, utc.hour, utc.minute, utc.second);
    buf[n] = 0;
...
static inline mln_lang_ctx_sys_exec_t *mln_lang_ctx_sys_exec_new(mln_lang_ctx_t *ctx, int fd)
{
    ...
    if ((se = (mln_lang_ctx_sys_exec_t *)mln_alloc_m(ctx->pool, sizeof(mln_lang_ctx_sys_exec_t))) == NULL) {
        return NULL;
    }
    se->ctx = ctx;
    se->fd = fd;
...
static void mln_lang_ctx_resource_sys_exec_free(mln_lang_ctx_sys_exec_t *cse)
{
    ...
    if ((tree = mln_lang_resource_fetch(cse->ctx->lang, "sys_exec")) != NULL) {
        mln_tcp_conn_fd_set(&se.conn, cse->fd);
        rn = mln_rbtree_search(tree, &se);
...
static mln_lang_var_t *mln_lang_sys_exec_process(mln_lang_ctx_t *ctx)
{
    mln_lang_var_t *ret_var = NULL, *pid_var;
    mln_string_t v1 = mln_string("cmd"), v2 = mln_string("bufsize"), v3 = mln_string("pid");
    mln_lang_symbol_node_t *sym;
    mln_string_t *cmd;
    mln_s64_t bufsize;
    mln_lang_ctx_sys_exec_t *cse;
    ...
    if (mln_lang_var_val_type_get(sym->data.var) == M_LANG_VAL_TYPE_NIL) {
        return mln_lang_sys_exec_process_info(ctx);
    }
    if (mln_lang_var_val_type_get(sym->data.var) != M_LANG_VAL_TYPE_STRING) {
        mln_lang_errmsg(ctx, "Invalid type of argument 1.");
        return NULL;
    ...
    if ((cse = mln_lang_ctx_resource_fetch(ctx, "sys_exec")) == NULL) {
        return ret_var;
    }
    ...
    if (!CreateProcess(NULL, (char *)cmd->data, NULL, NULL, TRUE, 0, NULL, NULL, &si, &pi)) {
        mln_lang_errmsg(ctx, "CreateProcess failed.");
        mln_socket_close(fds[0]);
        mln_socket_close(fds[1]);
        mln_lang_var_free(ret_var);
        return NULL;
    }
    pid = (mln_s64_t)(pi.dwProcessId);
    mln_lang_var_set_int(pid_var, pid);
...
static inline mln_lang_var_t *mln_lang_sys_exec_process_info(mln_lang_ctx_t *ctx)
{
    ...
    if (mln_rbtree_iterate(tree, mln_lang_sys_exec_process_info_iterate, arr) < 0) {
        return NULL;
    }
    return ret_var;
}
...
static int mln_lang_sys_exec_process_info_iterate(mln_rbtree_node_t *node, void *udata)
{
    ...
    mln_lang_var_value_set(ctx, v, _new);
    mln_lang_var_free(_new);
    arr = mln_lang_var_val_get(v)->data.array;

    if ((_new = mln_lang_var_create_string(ctx, &cmd_key, NULL)) == NULL) {
        mln_lang_errmsg(ctx, "No memory.");
        return -1;
    }
    ...
    mln_lang_var_set_string(v, mln_string_ref(se->cmd));

    if ((_new = mln_lang_var_create_string(ctx, &pid_key, NULL)) == NULL) {
        mln_lang_errmsg(ctx, "No memory.");
        return -1;
    }
    v = mln_lang_array_get(ctx, arr, _new);
