...
static void mln_lang_mysql_timeout_free(mln_lang_mysql_timeout_t *lmt)
{
    ...
    while ((lm = lmt->head) != NULL) {
        ...
        mln_event_fd_set(lmt->ctx->lang->ev, lm->fd_signal, M_EV_CLR, M_EV_UNLIMITED, NULL, NULL);
...
static void mln_lang_mysql_connect_test(mln_event_t *ev, int fd, void *data)
{
    ...
    if (status == NET_ASYNC_ERROR) {
        mln_event_fd_set(ctx->lang->ev, mysql->fd_signal, M_EV_CLR, M_EV_UNLIMITED, NULL, NULL);
        ...
        mln_event_fd_set(ctx->lang->ev, mysql->fd_signal, M_EV_SEND|M_EV_ONESHOT|M_EV_NONBLOCK, M_EV_UNLIMITED, mysql, mln_lang_mysql_connect_test);
    } else {
        mln_event_fd_set(ctx->lang->ev, mysql->fd_signal, M_EV_CLR, M_EV_UNLIMITED, NULL, NULL);
        mln_lang_mysql_chain_del(&(lmt->head), &(lmt->tail), mysql);
...
static mln_lang_var_t *mln_lang_mysql_connect_process(mln_lang_ctx_t *ctx)
{
    ...
    lmt = (mln_lang_mysql_timeout_t *)mln_lang_ctx_resource_fetch(ctx, "mysql");
    ASSERT(lmt != NULL);
    if (mln_event_fd_set(ctx->lang->ev, mysql->fd_signal, M_EV_SEND|M_EV_ONESHOT|M_EV_NONBLOCK, M_EV_UNLIMITED, mysql, mln_lang_mysql_connect_test) < 0) {
        mln_lang_errmsg(ctx, "No memory.");
        mln_lang_mysql_free(mysql);
        return NULL;
    }
...
static void mln_lang_mysql_free_test(mln_event_t *ev, int fd, void *data)
{
    ...
    if (status == NET_ASYNC_NOT_READY) {
        mln_event_fd_set(ctx->lang->ev, mysql->fd_signal, M_EV_SEND|M_EV_ONESHOT|M_EV_NONBLOCK, M_EV_UNLIMITED, mysql, mln_lang_mysql_free_test);
    } else {
        mysql->result = NULL;
        mln_event_fd_set(ctx->lang->ev, mysql->fd_signal, M_EV_CLR, M_EV_UNLIMITED, NULL, NULL);
        mln_lang_mysql_chain_del(&(lmt->head), &(lmt->tail), mysql);
...
static void mln_lang_mysql_fetch_test(mln_event_t *ev, int fd, void *data)
{
    ...
    if (status != NET_ASYNC_COMPLETE) {
        mln_event_fd_set(ctx->lang->ev, mysql->fd_signal, M_EV_SEND|M_EV_ONESHOT|M_EV_NONBLOCK, M_EV_UNLIMITED, mysql, mln_lang_mysql_fetch_test);
...
static void mln_lang_mysql_result_test(mln_event_t *ev, int fd, void *data)
{
    ...
    if (status == NET_ASYNC_NOT_READY) {
        mln_event_fd_set(ctx->lang->ev, mysql->fd_signal, M_EV_SEND|M_EV_ONESHOT|M_EV_NONBLOCK, M_EV_UNLIMITED, mysql, mln_lang_mysql_result_test);
    } else if (status == NET_ASYNC_ERROR) {
        mln_event_fd_set(ctx->lang->ev, mysql->fd_signal, M_EV_CLR, M_EV_UNLIMITED, NULL, NULL);
        mln_lang_mysql_chain_del(&(lmt->head), &(lmt->tail), mysql);
        mln_lang_ctx_continue(ctx);
...
static void mln_lang_mysql_query_test(mln_event_t *ev, int fd, void *data)
{
    ...
    if (status == NET_ASYNC_ERROR) {
        ...
        mln_event_fd_set(ctx->lang->ev, mysql->fd_signal, M_EV_SEND|M_EV_ONESHOT|M_EV_NONBLOCK, M_EV_UNLIMITED, mysql, mln_lang_mysql_query_test);
    } else {
        mln_event_fd_set(ctx->lang->ev, mysql->fd_signal, M_EV_SEND|M_EV_ONESHOT|M_EV_NONBLOCK, M_EV_UNLIMITED, mysql, mln_lang_mysql_result_test);
    }
    mln_lang_mutex_unlock(ctx->lang);
...
static mln_lang_var_t *mln_lang_mysql_execute_process(mln_lang_ctx_t *ctx)
{
    ...
    lmt = (mln_lang_mysql_timeout_t *)mln_lang_ctx_resource_fetch(ctx, "mysql");
    ASSERT(lmt != NULL);
    if (mln_event_fd_set(ctx->lang->ev, mysql->fd_signal, M_EV_SEND|M_EV_ONESHOT|M_EV_NONBLOCK, M_EV_UNLIMITED, mysql, mln_lang_mysql_query_test) < 0) {
        mln_lang_errmsg(ctx, "No memory.");
        return NULL;
    }
    if ((ret_var = mln_lang_var_create_false(ctx, NULL)) == NULL) {
