...
/*tcp*/
MLN_CHAIN_FUNC_DECLARE(static inline, mln_lang_tcp, mln_lang_tcp_t,);
MLN_CHAIN_FUNC_DEFINE(static inline, mln_lang_tcp, mln_lang_tcp_t, prev, next);
...
static int mln_lang_network_tcp_listen(mln_lang_ctx_t *ctx, mln_lang_object_t *obj);
static int mln_lang_network_tcp_accept(mln_lang_ctx_t *ctx, mln_lang_object_t *obj);
static int mln_lang_network_tcp_send(mln_lang_ctx_t *ctx, mln_lang_object_t *obj);
static int mln_lang_network_tcp_recv(mln_lang_ctx_t *ctx, mln_lang_object_t *obj);
static int mln_lang_network_tcp_connect(mln_lang_ctx_t *ctx, mln_lang_object_t *obj);
static int mln_lang_network_tcp_shutdown(mln_lang_ctx_t *ctx, mln_lang_object_t *obj);
static int mln_lang_network_tcp_close(mln_lang_ctx_t *ctx, mln_lang_object_t *obj);
...
/*udp*/
MLN_CHAIN_FUNC_DECLARE(static inline, mln_lang_udp, mln_lang_udp_t,);
MLN_CHAIN_FUNC_DEFINE(static inline, mln_lang_udp, mln_lang_udp_t, prev, next);
...
static int mln_lang_network_udp_create(mln_lang_ctx_t *ctx, mln_lang_object_t *obj);
static mln_lang_var_t *mln_lang_network_udp_create_process(mln_lang_ctx_t *ctx);
static int mln_lang_network_udp_close(mln_lang_ctx_t *ctx, mln_lang_object_t *obj);
static mln_lang_var_t *mln_lang_network_udp_close_process(mln_lang_ctx_t *ctx);
static int mln_lang_network_udp_send(mln_lang_ctx_t *ctx, mln_lang_object_t *obj);
...
mln_lang_var_t *init(mln_lang_ctx_t *ctx, mln_conf_t *cf)
{
    ...
    if (mln_lang_network(ctx, mln_lang_var_val_get(obj)->data.obj) < 0) {
...
static int mln_lang_network_resource_register(mln_lang_ctx_t *ctx)
{
    ...
    if ((tcp_set = mln_lang_resource_fetch(ctx->lang, "tcp")) == NULL) {
        ...
        if (mln_lang_resource_register(ctx->lang, "tcp", tcp_set, (mln_lang_resource_free)mln_rbtree_free) < 0) {
            mln_lang_errmsg(ctx, "No memory.");
            mln_rbtree_free(tcp_set);
            return -1;
        }
    }
    if ((udp_set = mln_lang_resource_fetch(ctx->lang, "udp")) == NULL) {
        ...
        if (mln_lang_resource_register(ctx->lang, "udp", udp_set, (mln_lang_resource_free)mln_rbtree_free) < 0) {
            mln_lang_errmsg(ctx, "No memory.");
            mln_rbtree_free(udp_set);
            return -1;
        }
    }

    mln_lang_ctx_tcp_t *lct;
    if ((lct = mln_lang_ctx_resource_fetch(ctx, "tcp")) == NULL) {
    ...
    if ((lct = mln_lang_ctx_resource_fetch(ctx, "udp")) == NULL) {
...
static mln_lang_tcp_t *mln_lang_network_tcp_resource_fetch(mln_lang_t *lang, int fd)
{
    ...
    return (mln_lang_tcp_t *)mln_rbtree_node_data_get(rn);
}
...
static int mln_lang_network_tcp_listen(mln_lang_ctx_t *ctx, mln_lang_object_t *obj)
{
    ...
    mln_string_t funcname = mln_string("tcp_listen");
...
static int mln_lang_network_tcp_accept(mln_lang_ctx_t *ctx, mln_lang_object_t *obj)
{
    ...
    mln_string_t funcname = mln_string("tcp_accept");
...
static mln_lang_var_t *mln_lang_network_tcp_accept_process(mln_lang_ctx_t *ctx)
{
    ...
    tcp->timeout = timeout;
    if (mln_event_fd_set(ctx->lang->ev, fd, M_EV_RECV|M_EV_NONBLOCK|M_EV_ONESHOT,timeout, tcp, mln_lang_network_tcp_accept_handler) < 0) {
        mln_lang_errmsg(ctx, "No memory.");
        mln_lang_var_free(ret_var);
        return NULL;
    }
    if (timeout >= 0) {
        mln_event_fd_timeout_handler_set(ctx->lang->ev, fd, tcp, mln_lang_network_tcp_timeout_handler);
    }
    tcp->recving = 1;
...
static int mln_lang_network_tcp_recv(mln_lang_ctx_t *ctx, mln_lang_object_t *obj)
{
    ...
    mln_lang_func_detail_t *func;
    mln_string_t v1 = mln_string("fd"), v2 = mln_string("timeout");
    mln_string_t funcname = mln_string("tcp_recv");
...
static mln_lang_var_t *mln_lang_network_tcp_recv_process(mln_lang_ctx_t *ctx)
{
    ...
    if (mln_event_fd_set(ctx->lang->ev, fd, M_EV_RECV|M_EV_NONBLOCK|M_EV_ONESHOT, timeout, tcp, mln_lang_network_tcp_recv_handler) < 0) {
        mln_lang_errmsg(ctx, "No memory.");
        mln_lang_var_free(ret_var);
        return NULL;
    }
...
static int mln_lang_network_tcp_send(mln_lang_ctx_t *ctx, mln_lang_object_t *obj)
{
    ...
    mln_string_t funcname = mln_string("tcp_send");
...
static mln_lang_var_t *mln_lang_network_tcp_send_process(mln_lang_ctx_t *ctx)
{
    ...
    if (mln_event_fd_set(ctx->lang->ev, fd, M_EV_SEND|M_EV_NONBLOCK|M_EV_ONESHOT, M_EV_UNLIMITED, tcp, mln_lang_network_tcp_send_handler) < 0) {
        mln_lang_errmsg(ctx, "No memory.");
        mln_lang_var_free(ret_var);
        return NULL;
    }
    if ((c = mln_chain_new(mln_tcp_conn_pool_get(&(tcp->conn)))) == NULL) {
        mln_lang_errmsg(ctx, "No memory.");
        mln_event_fd_set(ctx->lang->ev, fd, M_EV_CLR, M_EV_UNLIMITED, NULL, NULL);
        mln_lang_var_free(ret_var);
        return NULL;
    }
    if ((b = mln_buf_new(mln_tcp_conn_pool_get(&(tcp->conn)))) == NULL) {
        mln_lang_errmsg(ctx, "No memory.");
    ...
    c->buf = b;
    if ((buf = (mln_u8ptr_t)mln_alloc_m(mln_tcp_conn_pool_get(&(tcp->conn)), data->len)) == NULL) {
        mln_lang_errmsg(ctx, "No memory.");
        mln_chain_pool_release(c);
        mln_event_fd_set(ctx->lang->ev, fd, M_EV_CLR, M_EV_UNLIMITED, NULL, NULL);
        mln_lang_var_free(ret_var);
...
static int mln_lang_network_tcp_connect(mln_lang_ctx_t *ctx, mln_lang_object_t *obj)
{
    ...
    mln_string_t funcname = mln_string("tcp_connect");
...
static mln_lang_var_t *mln_lang_network_tcp_connect_process(mln_lang_ctx_t *ctx)
{
     ...
     */
    if (mln_event_fd_set(ctx->lang->ev, \
                         fd, \
    ...
    if (timeout != M_EV_UNLIMITED) {
        mln_event_fd_timeout_handler_set(tcp->lang->ev, fd, tcp, mln_lang_network_tcp_connect_timeout_handler);
    }
    mln_lang_ctx_tcp_resource_add(ctx, tcp);
    mln_lang_ctx_suspend(ctx);
...
static int mln_lang_network_tcp_shutdown(mln_lang_ctx_t *ctx, mln_lang_object_t *obj)
{
    ...
    mln_string_t funcname = mln_string("tcp_shutdown");
        ...
        if (tcp->send_closed) {
            ...
            if (tcp->recving) {
                if (tcp->sending) {
                    tcp->recving = 0;
                    tcp->recv_closed = 1;
                    mln_event_fd_set(tcp->lang->ev, fd, M_EV_SEND|M_EV_NONBLOCK|M_EV_ONESHOT, M_EV_UNLIMITED, tcp, mln_lang_network_tcp_send_handler);
                } else {
                    mln_event_fd_set(tcp->lang->ev, fd, M_EV_CLR, M_EV_UNLIMITED, NULL, NULL);
                    mln_lang_ctx_continue(tcp->ctx);
                    mln_lang_ctx_tcp_resource_remove(tcp);
...
static int mln_lang_network_tcp_close(mln_lang_ctx_t *ctx, mln_lang_object_t *obj)
{
    ...
    mln_string_t funcname = mln_string("tcp_close");
    ...
    if (getsockopt(fd, SOL_SOCKET, SO_ERROR, &err, &len) >= 0) {
        ...
        if (err) {
            if (err == EINPROGRESS && (++tcp->retry <= MLN_LANG_NETWORK_TCP_CONNECT_RETRY)) {
                mln_event_fd_set(ev, fd, M_EV_SEND|M_EV_ERROR|M_EV_NONBLOCK|M_EV_ONESHOT, tcp->connect_timeout, tcp, mln_lang_network_tcp_connect_handler);
                if (tcp->connect_timeout != M_EV_UNLIMITED) {
                    mln_event_fd_timeout_handler_set(ev, fd, tcp, mln_lang_network_tcp_connect_timeout_handler);
                }
...
static void mln_lang_network_tcp_accept_handler(mln_event_t *ev, int fd, void *data)
{
    ...
    if ((connfd = accept(fd, (struct sockaddr *)addr, &len)) < 0) {
        ...
        if (mln_lang_network_get_addr(addr, ip, &port) < 0) {
            ...
            if ((newtcp = mln_lang_tcp_new(lang, connfd, ip, port)) == NULL) {
                mln_socket_close(connfd);
            } else {
                if ((ret_var = mln_lang_var_create_int(tcp->ctx, connfd, NULL)) == NULL) {
                    mln_lang_tcp_free(newtcp);
                } else {
                    if (mln_lang_network_tcp_resource_add(lang, newtcp) < 0) {
                        mln_lang_var_free(ret_var);
                        mln_lang_tcp_free(newtcp);
                    } else {
                        mln_lang_ctx_set_ret_var(tcp->ctx, ret_var);
...
static void mln_lang_network_tcp_recv_handler(mln_event_t *ev, int fd, void *data)
{
    ...
    if (rc == M_C_ERROR) {
        mln_lang_mutex_unlock(lang);
        /* do nothing */
    } else if (rc == M_C_CLOSED && mln_tcp_conn_head(&(tcp->conn), M_C_RECV) == NULL) {
        mln_lang_var_t *ret_var = mln_lang_var_create_true(tcp->ctx, NULL);
        if (ret_var == NULL) {
            mln_lang_mutex_unlock(lang);
            mln_event_fd_set(ev, fd, M_EV_SEND|M_EV_NONBLOCK|M_EV_ONESHOT, M_EV_UNLIMITED, tcp, mln_lang_network_tcp_recv_handler);
            return;
        }
        mln_lang_ctx_set_ret_var(tcp->ctx, ret_var);
    } else {
        c = mln_tcp_conn_head(&(tcp->conn), M_C_RECV);
        for (; c != NULL; c = c->next) {
        ...
        for (p = buf, c = mln_tcp_conn_head(&(tcp->conn), M_C_RECV); c != NULL; c = c->next) {
            if (c->buf == NULL) continue;
        ...
        if (ret_var == NULL) {
            mln_lang_mutex_unlock(lang);
            mln_event_fd_set(ev, fd, M_EV_RECV|M_EV_NONBLOCK|M_EV_ONESHOT, M_EV_UNLIMITED, tcp, mln_lang_network_tcp_recv_handler);
            return;
        }
        mln_lang_ctx_set_ret_var(tcp->ctx, ret_var);
...
static void mln_lang_network_tcp_send_handler(mln_event_t *ev, int fd, void *data)
{
    ...
    if (rc == M_C_FINISH || rc == M_C_NOTYET) {
        ...
        if (mln_tcp_conn_head(&(tcp->conn), M_C_SEND) != NULL) {
            mln_lang_mutex_unlock(lang);
            mln_event_fd_set(ev, fd, M_EV_SEND|M_EV_NONBLOCK|M_EV_ONESHOT, M_EV_UNLIMITED, tcp, mln_lang_network_tcp_send_handler);
            return;
        } else {
            mln_lang_var_t *ret_var = mln_lang_var_create_true(tcp->ctx, NULL);
...
static void mln_lang_tcp_free(mln_lang_tcp_t *lt)
{
    ...
    mln_event_fd_set(lt->lang->ev, mln_tcp_conn_fd_get(&(lt->conn)), M_EV_CLR, M_EV_UNLIMITED, NULL, NULL);
    mln_socket_close(mln_tcp_conn_fd_get(&(lt->conn)));
...
static int mln_lang_tcp_cmp(const mln_lang_tcp_t *lt1, const mln_lang_tcp_t *lt2)
{
    return mln_tcp_conn_fd_get(&(lt1->conn)) - mln_tcp_conn_fd_get(&(lt2->conn));
}
...
static mln_lang_udp_t *mln_lang_network_udp_resource_fetch(mln_lang_t *lang, int fd)
{
    ...
    return (mln_lang_udp_t *)mln_rbtree_node_data_get(rn);
}
...
static int mln_lang_network_udp_create(mln_lang_ctx_t *ctx, mln_lang_object_t *obj)
{
    ...
    mln_lang_func_detail_t *func;
    mln_string_t v1 = mln_string("host"), v2 = mln_string("service");
    mln_string_t funcname = mln_string("udp_create");
...
static int mln_lang_network_udp_close(mln_lang_ctx_t *ctx, mln_lang_object_t *obj)
{
    ...
    mln_lang_func_detail_t *func;
    mln_string_t v1 = mln_string("fd");
    mln_string_t funcname = mln_string("udp_close");
...
static int mln_lang_network_udp_send(mln_lang_ctx_t *ctx, mln_lang_object_t *obj)
{
    ...
    mln_string_t funcname = mln_string("udp_send");
...
static mln_lang_var_t *mln_lang_network_udp_send_process(mln_lang_ctx_t *ctx)
{
    ...
    if (mln_event_fd_set(ctx->lang->ev, fd, M_EV_SEND|M_EV_NONBLOCK|M_EV_ONESHOT, M_EV_UNLIMITED, udp, mln_lang_network_udp_send_handler) < 0) {
        mln_lang_errmsg(ctx, "No memory.");
        mln_lang_var_free(ret_var);
        return NULL;
    }
...
static void mln_lang_network_udp_send_handler(mln_event_t *ev, int fd, void *data)
{
    ...
    if (rc < 0 && err == EINTR) {
        mln_lang_mutex_unlock(lang);
        mln_event_fd_set(ev, fd, M_EV_SEND|M_EV_NONBLOCK|M_EV_ONESHOT, M_EV_UNLIMITED, udp, mln_lang_network_udp_send_handler);
        return;
    }
    if (rc >= 0) {
...
static int mln_lang_network_udp_recv(mln_lang_ctx_t *ctx, mln_lang_object_t *obj)
{
    ...
    mln_string_t funcname = mln_string("udp_recv");
...
static mln_lang_var_t *mln_lang_network_udp_recv_process(mln_lang_ctx_t *ctx)
{
    ...
    if (mln_event_fd_set(ctx->lang->ev, fd, M_EV_RECV|M_EV_NONBLOCK|M_EV_ONESHOT, udp->timeout, udp, mln_lang_network_udp_recv_handler) < 0) {
        mln_lang_errmsg(ctx, "No memory.");
        mln_lang_var_free(ret_var);
        return NULL;
    }
    if (timeout != M_EV_UNLIMITED) {
        mln_event_fd_timeout_handler_set(ctx->lang->ev, fd, udp, mln_lang_network_udp_timeout_handler);
    }
    udp->recving = 1;
    mln_lang_ctx_udp_resource_add(ctx, udp);
    mln_lang_ctx_suspend(ctx);
...
static void mln_lang_network_udp_recv_handler(mln_event_t *ev, int fd, void *data)
{
    ...
    if ((buf = (mln_u8ptr_t)malloc(udp->bufsize)) == NULL) {
        mln_event_fd_set(ev, fd, M_EV_RECV|M_EV_NONBLOCK|M_EV_ONESHOT, udp->timeout, udp, mln_lang_network_udp_recv_handler);
        if (udp->timeout != M_EV_UNLIMITED)
            mln_event_fd_timeout_handler_set(ev, fd, udp, mln_lang_network_udp_timeout_handler);
        mln_lang_mutex_unlock(lang);
        return;
    }
...
static void mln_lang_udp_free(mln_lang_udp_t *lu)
{
    ...
    mln_event_fd_set(lu->lang->ev, lu->fd, M_EV_CLR, M_EV_UNLIMITED, NULL, NULL);
